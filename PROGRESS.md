...
> **Status:** Walking skeleton complete — compiles and passes `go vet`
...
| LLM | Direct HTTP client to OpenAI-compatible API (no SDK) |
| Streaming | Server-Sent Events (SSE) |
| Container | Multi-stage Docker build (alpine), Docker Compose |
...
│   │   └── inference.go             # POST complete (streaming + non-streaming)
│   ├── middleware/
│   │   ├── auth.go                  # JWT validation, user context injection, role gating
...
├── docker-compose.yml               # Postgres (pgvector:pg16) + Redis (7-alpine)
...
**26 source files** | 5 direct dependencies | 0 external frameworks
...
- **Prompt length limit** — 32K character cap enforced server-side
...
1. **Tests** — Unit tests for auth, cache, orchestrator; integration tests with testcontainers
